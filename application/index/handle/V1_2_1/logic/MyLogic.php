...
class MyLogic
{
    ...
    function __construct( $api_version )
    {
        ...
        $model_path = $api_version."model\MyModel";
    ...
    public function is_bind($channels,$params,$users){
        ...
        $tel        = $this->handle->getTel($users,$channel);
    ...
    public function login_code($channels,$params,$users)
    {
        ...
        $tel        = $params['username'];
        $channel    = $channels['channel'];
        $res        = $this->handle->CountCode($tel,strtotime(date('Y-m-d')),$channel);
        ...
        if(isMobile($tel))
        {
            ...
            if($res[0]['count'] >= 3 && $res[0]['count']<10)
            {
            ...
            if($res[0]['count']>=10) error(50000,'您今日发送验证码次数已用完！');
            $sms = new SmsSend;
            $code= rand(1000,9999);
            $msg = '您的验证码是'.$code;
            ...
            if($result->errmsg == "OK")
            {
                $this->handle->saveCode($code,$channel,$tel,strtotime(date('Y-m-d')));
                success(['operation'=>1]);
            }
