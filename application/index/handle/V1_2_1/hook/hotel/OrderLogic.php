...
namespace app\index\handle\V1_2_1\hook\hotel;
...
class OrderLogic
{
    ...
    public static function create($data)
    {
        ...
        $room       = OrderModel::getRoomType($channel, $shop_id, $room_id);
        if ($count<$room['min_limit']||$count>$room['max_limit']) {
            error(40000, '购买数量有误！');
        }
        $shop_arr = OrderModel::getShop($room['shop_id']);
        if (empty($shop_arr)) {
            error(50000, '没有找到店铺');
        }
        $shop = $shop_arr[0];
        ...
        $where['date'] = $data_map;
        $hotel_booking =OrderModel::getBooking($where);
        ...
        foreach ($hotel_booking as $item) {
            $getOrderCount  = OrderModel::getOrderCount($channel, $item['room'], $item['date'], 2, NOW-1800);
