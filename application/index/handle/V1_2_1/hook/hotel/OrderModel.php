...
class OrderModel
{
   ...
   {
       return Db::table('hotel_room_type')->where(['id'=>$room_id,'status'=>1,'channel'=>$channel,'shop_id'=>$shop_id])->find();
   }
   ...
   static function create($data,$snap)
   {
    ...
    {
        return Db::table('hotel_room_type')->where(['id'=>$id])->field('hid')->find();
    }
}