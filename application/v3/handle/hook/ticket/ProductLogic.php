...
namespace app\v3\handle\hook\ticket;

use app\v3\model\Shop\Product;
use app\v3\model\Shop\ProductTicketBooking;
use app\v3\model\Shop\ProductTicketItem;
use app\v3\model\Shop\ProductTicketTag;
use think\db\Query;
...
class ProductLogic
{
    public static function lists($data)
    {
        ...
        $productList = Product::where('channel', $channel['channel_id'])
    ...
    public static function detail($data)
    {
        ...
        $product = Product::where('channel', $channel['channel_id'])->where('shop_id', $channel['shop_id'])
            ->where('id', $productId)->where('type', $type)->where('status', Product::STATUS_VALID)
        ...
        $data = [
            ...
            'price_min' => empty(min($product->ticket_items->column('booking_min') ? $product->ticket_items->column('booking_min') : [0])) ? 0 : floatval(min($product->ticket_items->column('booking_min') ? $product->ticket_items->column('booking_min') : [0])),
